    ...
    leagues: {
      top: {
        ...
        feeds: [
          ...
          { name: "The Guardian", url: "https://www.theguardian.com/football/rss", ua: "browser" },
          // Sky's feed may mix in other sports (cricket, F1, rugby), so
          // linkMustInclude keeps only items whose article URL is under
          // skysports.com/football/ -- harmless if the feed is already
          // football-only, protective if it isn't.
          {
            name: "Sky Sports",
            url: "https://www.skysports.com/rss/11095",
            linkMustInclude: "/football/",
            ua: "browser",
          },
        ],
      },
      "premier-league": {
        ...
        feeds: [
          { name: "The Guardian", url: "https://www.theguardian.com/football/premierleague/rss", ua: "browser" },
        ],
      },
      "la-liga": {
        label: "La Liga",
        feeds: [
          { name: "The Guardian", url: "https://www.theguardian.com/football/laligafootball/rss", ua: "browser" },
        ],
      },
      "champions-league": {
        label: "Champions League",
        feeds: [
          { name: "The Guardian", url: "https://www.theguardian.com/football/championsleague/rss", ua: "browser" },
        ],
      },
      "serie-a": {
        label: "Serie A",
        feeds: [
          { name: "The Guardian", url: "https://www.theguardian.com/football/serieafootball/rss", ua: "browser" },
        ],
      },
      mls: {
        label: "MLS",
        feeds: [{ name: "The Guardian", url: "https://www.theguardian.com/football/mls/rss", ua: "browser" }],
      },
    },
  },
};
...
// Different outlets accept different clients, and the rules are close to
// opposite of intuitive. ESPN *rejects* requests that claim to be a
// browser but come from a datacenter (anti-spoofing) while happily
// serving a transparent bot UA. The Guardian appears to block unknown
// bots (or possibly datacenter IPs entirely). So headers are chosen
// per-feed: the honest bot identity for feeds proven to accept it, the
// browser identity only where the bot identity demonstrably fails.
const BOT_HEADERS = {
...
async function fetchFeed({ name, url, linkMustInclude, ua }) {
  try {
    const res = await fetch(url, {
      // Server-side cache: re-fetch at most every 5 minutes.
      next: { revalidate: 300 },
      headers: ua === "browser" ? BROWSER_HEADERS : BOT_HEADERS,
    });
    if (!res.ok) {
