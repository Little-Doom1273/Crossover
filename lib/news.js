  ...
  soccer: {
    ...
    leagues: {
      ...
      transfers: {
        ...
        feeds: [
          { name: "The Guardian", guardianSection: "football/transfer-window" },
          // Per-league transfer queries via Google News search RSS (built
          // for programmatic access, unlike the outlets' own blocked
          // feeds). This deliberately covers all big-five leagues so the
          // tab isn't just Premier League news, and pulls from every
          // outlet Google indexes -- Sky, Marca, Football Italia, etc.
          // "when:2d" keeps each query to the last two days. The real
          // publisher name is extracted per item; links route through
          // Google's redirect to the original article.
          gnewsFeed("Premier League transfers", '"Premier League" transfer when:2d'),
          gnewsFeed("La Liga transfers", '"La Liga" transfer when:2d'),
          gnewsFeed("Serie A transfers", '"Serie A" transfer when:2d'),
          gnewsFeed("Bundesliga transfers", "Bundesliga transfer when:2d"),
          gnewsFeed("Ligue 1 transfers", '"Ligue 1" transfer when:2d'),
        ],
      },
      "premier-league": {
...
// Builds a Google News search-RSS feed config. The configured name is
// only a fallback label; each item's real publisher is extracted in
// normalizeGoogleNewsItem below.
function gnewsFeed(name, query) {
  return {
    name,
...
// Google News RSS items need cleanup: titles end with " - Publisher"
// (stripped, and used as the item's source so readers see the real
// outlet, e.g. "Sky Sports" or "Marca"), and descriptions are HTML
// that just repeats the headline (dropped).
export function normalizeGoogleNewsItem(item) {
  const { rawSourceName, ...rest } = item;
  const suffixMatch = item.title.match(/\s+-\s+([^-]+)$/);
  const publisher = (rawSourceName || (suffixMatch ? suffixMatch[1] : "")).trim();
  return {
    ...rest,
    title: item.title.replace(/\s+-\s+[^-]+$/, "").trim(),
    summary: "",
    source: publisher || rest.source,
  };
}

async function fetchFeed(feed) {
  if (feed.guardianSection) return fetchGuardianFeed(feed);
  const { name, url, linkMustInclude, googleNews } = feed;
  try {
    const res = await fetch(url, {
    ...
    const mapped = list
      .map((item) => ({
        ...
        // Google News includes the real publisher in a per-item <source>
        // tag; carry it through for normalizeGoogleNewsItem.
        ...(googleNews
          ? {
              rawSourceName:
      ...
      .map((item) => (googleNews ? normalizeGoogleNewsItem(item) : item));
    if (mapped.length === 0) {
