...
export function getTodaysTopStories(allItems, { limit = 10, hours = 24 } = {}) {
  ...
  // Freshness bonus: outlet count is still the main signal, but breaking
  // news gets a thumb on the scale. A story from the last 2 hours gets
  // +1 (worth one extra outlet of coverage); the last 6 hours gets +0.5.
  // So 2 outlets covering something from 20 minutes ago now outranks
  // 2 outlets on a story from 20 hours ago -- and can even edge out a
  // 3-outlet story that's nearly a day old.
  function freshnessBonus(published) {
  ...
  const ranked = clusters
    .map((cluster) => {
      ...
      const top = sorted[0];
      // Bulk re-stamped timestamps (timeReliable === false) can't be
      // trusted as "breaking news" signals, so they earn no bonus.
      const bonus = top.timeReliable === false ? 0 : freshnessBonus(top.published);
      return {
        headline: top.title,
        link: top.link,
        summary: top.summary,
        sportKey: top.sportKey,
        image: top.image || cluster.items.find((i) => i.image)?.image || null,
        published: top.published,
        timeReliable: top.timeReliable,
        sourceCount: cluster.sources.size,
        sources: [...cluster.sources],
        score: cluster.sources.size + bonus,
      };
    })
    .sort((a, b) => {
